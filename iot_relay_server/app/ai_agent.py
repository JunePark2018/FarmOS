...
from app.ai_agent_prompts import SYSTEM_PROMPT, build_user_prompt
...
_last_daily_reset: str = ""  # 마지막 일일 리셋 날짜 (YYYY-MM-DD)
...
control_state: dict = {
    ...
    "irrigation": {
        "valve_open": False, "daily_total_L": 0.0,
        "last_watered": None, "nutrient": {"N": 1.0, "P": 1.0, "K": 1.0},
    },
...
def _apply_rules(sensor_data: dict, weather: dict) -> list[dict]:
    ...
    # (직전에 강수로 창문이 닫혔고, 지금 강수가 끝난 경우에만)
    ...
    if temp > 35:
        ...
        decisions.append(_make_decision("ventilation", control_state["ventilation"], f"내부 온도 {temp}C — 긴급 냉각. 창문 100%, 팬 최대.", "emergency", "rule"))
        ...
        decisions.append(_make_decision("ventilation", control_state["ventilation"], f"내부 {temp}C > 외부 {ext_temp}C. 자연환기 {pct}%.", "high", "rule"))
    ...
    if humidity > 90 and control_state["ventilation"]["fan_speed"] < 1500:
        ...
        if not (is_night and ext_temp is not None and ext_temp < 5):
            control_state["ventilation"]["window_open_pct"] = max(control_state["ventilation"]["window_open_pct"], 50)
        decisions.append(_make_decision("ventilation", control_state["ventilation"], f"습도 {humidity}% — 결로/병해 방지 환기.", "high", "rule"))
    ...
    if (precip > 0 or precip_type != "없음") and control_state["ventilation"]["window_open_pct"] > 0:
        ...
        decisions.append(_make_decision("ventilation", control_state["ventilation"], f"강수 감지({precip_type} {precip}mm) — 창문 닫음.", "high", "rule"))
    ...
    if soil < 30:
        ...
        decisions.append(_make_decision("irrigation", {"water_amount_L": water, "nutrient_ratio": crop_profile["nutrient_ratio"]}, f"토양수분 {soil}% — 긴급 관수 {water}L.", "emergency", "rule"))

    if is_night and ext_temp is not None and ext_temp < 5:
        control_state["shading"]["insulation_pct"] = 100
        control_state["ventilation"]["window_open_pct"] = 0
        decisions.append(_make_decision("shading", control_state["shading"], f"야간 외부 {ext_temp}C — 동해 방지 보온커튼 100%.", "emergency", "rule"))
        ...
        decisions.append(_make_decision("shading", control_state["shading"], f"야간 외부 {ext_temp}C — 보온커튼 {ins}%.", "medium", "rule"))

    if is_night and control_state["lighting"]["on"]:
        control_state["lighting"] = {"on": False, "brightness_pct": 0}
        decisions.append(_make_decision("lighting", control_state["lighting"], "야간 암기 유지 — 조명 OFF.", "low", "rule"))
...
def _apply_llm_result(parsed: dict) -> list[dict]:
    ...
    if "ventilation" in parsed:
        ...
        control_state["ventilation"] = {"window_open_pct": max(0, min(100, int(v.get("window_open_pct", 0)))), "fan_speed": max(0, min(3000, int(v.get("fan_speed", 0))))}
    ...
    if "irrigation" in parsed:
        ...
        if water > 0:
            ...
            control_state["irrigation"]["nutrient"] = {"N": float(nr.get("N", 1.0)), "P": float(nr.get("P", 1.0)), "K": float(nr.get("K", 1.0))}
        else:
            control_state["irrigation"]["valve_open"] = False
    ...
    if "lighting" in parsed:
        lt = parsed["lighting"]
        control_state["lighting"] = {"on": bool(lt.get("on", False)), "brightness_pct": max(0, min(100, int(lt.get("brightness_pct", 0))))}
        decisions.append(_make_decision("lighting", control_state["lighting"], reason, "low", "llm"))

    if "shading" in parsed:
        sh = parsed["shading"]
        control_state["shading"] = {"shade_pct": max(0, min(100, int(sh.get("shade_pct", 0)))), "insulation_pct": max(0, min(100, int(sh.get("insulation_pct", 0))))}
...
def _make_decision(control_type: str, action: dict, reason: str, priority: str, source: str) -> dict:
    decision = {"id": str(uuid4()), "timestamp": datetime.now(KST).isoformat(), "control_type": control_type, "action": action, "reason": reason, "priority": priority, "source": source}
...
def _has_significant_change(new_data: dict) -> bool:
    ...
    for key in ["temperature", "humidity", "light_intensity"]:
        old, new = _last_sensor_data.get(key, 0), new_data.get(key, 0)
        if old == 0:
            if new != 0: return True
            continue
        if abs(new - old) / abs(old) > 0.05: return True
    old_soil = _last_sensor_data.get("soil_moisture") or 50
    new_soil = new_data.get("soil_moisture") or 50
    if abs(new_soil - old_soil) > 3: return True
    return False
...
async def process_sensor_data(raw_sensors: dict) -> list[dict]:
    ...
    sensor_data = {"temperature": filtered["temperature"], "humidity": filtered["humidity"], "light_intensity": filtered["light_intensity"], "soil_moisture": filtered.get("soil_moisture")}
...
def update_crop_profile(data: dict) -> dict:
    global crop_profile
    crop_profile = {
        "name": data["name"], "growth_stage": data["growth_stage"],
        "optimal_temp": data["optimal_temp"], "optimal_humidity": data["optimal_humidity"],
...
def override_control(control_type: str, values: dict, reason: str) -> dict:
    ...
    return _make_decision(control_type, values, f"수동 오버라이드: {reason}", "high", "manual")
