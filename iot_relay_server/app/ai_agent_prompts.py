...
def build_user_prompt(
    sensor_data: dict, weather: dict, crop_profile: dict,
    control_state: dict, reliability: dict,
) -> str:
    ...
    for fc in forecasts:
        forecast_text += (
            ...
            f"기온 {fc.get('temperature', '?')}C, 습도 {fc.get('humidity', '?')}%, "
            f"하늘 {fc.get('sky', '?')}, 강수확률 {fc.get('precipitation_prob', 0)}%\n"
        )
...
## 현재 기상 (외부)
- 기온: {weather_current.get('temperature', '?')}C, 습도: {weather_current.get('humidity', '?')}%
...
## 시간: {now.strftime('%H:%M')} KST ({'낮' if is_daytime else '밤'})

최적의 제어 명령을 JSON으로 출력하세요."""
